...
export async function publishFirstListing({ data, logoUrl }: FirstPublishProps) {
  ...
  try {
    if (!data.user_Id) {
      ...
      const { data: usersData, error: usersError } = await supabase.from('users').select('*').eq('id', data.user_Id).single();
      ...
      if (jobPostError || !jobPostData) {
        console.log('jobPostError', jobPostError);
      ...
      if (companyUpdateError) {
        console.log('companyerror', companyUpdateError.message);
        return { type: 'error' as const, message: companyUpdateError.message };
      }
      ...
      if (data.contactName !== usersData.contactName && data.contactPhone !== usersData.contactPhone) {
        ...
        if (usersUpdateError) {
          console.log('usersUpdateError name and phone', usersUpdateError.message);

          return { type: 'error' as const, message: usersUpdateError.message };
        }
      } else if (data.contactName !== usersData.contactName) {
        const { error: usersUpdateError } = await supabase.from('users').update({ contactName: data.contactName }).eq('id', usersData.id);

        if (usersUpdateError) {
          console.log('usersUpdateError name', usersUpdateError.message);
